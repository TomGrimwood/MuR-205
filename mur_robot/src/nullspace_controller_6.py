...
def controller(data):
	...
	if diff_time.to_nsec()>5000000:
		...
		## New
		#print "target_Trafo: ",target_Trafo[0][3],target_Trafo[1][3]
		...
		error_x =target_x - current_point_world[0][3]
		error_y =target_y - current_point_world[1][3]
		error_g =target_gamma - gamma
		print "current gamma: ",gamma,"error gamma: ",round(error_g,5)

		if error_g< -0.8*2*math.pi:
			error_g=error_g+2*math.pi
		elif error_g> 0.8*2*math.pi:
			error_g=error_g-2*math.pi
		...
		e_sum_x_array.insert(0,rotated_errors[0])
		e_sum_y_array.insert(0,rotated_errors[1])
		...
		print "P: ",round(rotated_errors[0],4),round(rotated_errors[1],4)
		print "I: ",round(e_sum_x,4),round(e_sum_y,4)
		print "D: ",round(error_diff_x,4),round(error_diff_y,4)
...
def nullspaceMotion():
	...
	rospy.set_param("target_x",float(target_Trafo[0][3]))
	rospy.set_param("target_y",float(target_Trafo[1][3]))
	...
	rospy.Subscriber("/odom", Odometry, controller,queue_size=1)
	rospy.spin()
