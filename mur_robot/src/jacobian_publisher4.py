...
class JacobianPublisher():
	...
	def computeJointSpeeds(self,J):

		cartesian_velocities=rospy.get_param("mur/cartesian_velocities")
		
		
		#cartesian_velocities1=cartesian_velocities
		#cartesian_velocities1[5]=cartesian_velocities[1]
		#cartesian_velocities1[1]=0

		
		
		J_inv=np.linalg.pinv(J)

		J_transposed=J.transpose()

		J_JT=np.dot(J,J_transposed)

		#rank_J = np.linalg.matrix_rank(J)
		#print "Rank of J: ",rank_J

		u, s, vh = np.linalg.svd(J_JT)

		

		q_dot=np.dot(J_inv,cartesian_velocities)
		...
		inv_Dexterity_index=math.sqrt((s[0]**2)/(s[5]**2))

		print "inv_Dexterity_index: ",inv_Dexterity_index
	...
	def callback(self,joint_states,odom,wheel_vel):
		...
		joint_speeds,wheel_speeds= self.computeJointSpeeds(J)
