...
class JacobianPublisher():
	...
	def callback(self,joint_states,odom,wheel_vel):
		...
		world_R_mir_3x3_large=tf.transformations.euler_matrix(orientation_euler[0],orientation_euler[1],orientation_euler[2])
		world_R_mir_3x3=world_R_mir_3x3_large[:-1,0:-1]
		...
		vec_mir_base_ur_tcp_rotated=np.dot(world_R_mir_3x3_large,vec_mir_base_ur_tcp)
		...
		'''
		skew_matrix_small=np.array([
							[0,vec_mir_base_ur_tcp_rotated[2],-vec_mir_base_ur_tcp_rotated[1]],
							[-vec_mir_base_ur_tcp_rotated[2],0,vec_mir_base_ur_tcp_rotated[0]],
							[vec_mir_base_ur_tcp_rotated[1],-vec_mir_base_ur_tcp_rotated[0],0]
							])
		...
		mir_R_ur_2=np.concatenate((zeros_3_3,eye),1)
		...
		print"world_R_mir_3x3: \n",world_R_mir_3x3
		
		print"\norientation_euler: \n",orientation_euler

		world_R_mir_1=np.concatenate((world_R_mir_3x3,zeros_3_3),1)
		world_R_mir_2=np.concatenate((zeros_3_3,eye),1)
